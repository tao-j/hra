...
%     break
    fid = fopen(sprintf('./read_data/%dt1.txt', pps(pp)),'w');
    fprintf(fid, 'btl-random-opt\n');
    fclose(fid);
    dlmwrite(sprintf('./read_data/%dt1.txt', pps(pp)),mean(res_btl, 3),'-append');
    fid = fopen(sprintf('./read_data/%dt1.txt', pps(pp)),'a+');
    fprintf(fid, 'crowdbt-opt\n');
    fclose(fid);
    dlmwrite(sprintf('./read_data/%dt1.txt', pps(pp)),mean(res_gbtl, 3),'-append');
end